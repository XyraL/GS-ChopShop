--========================================
-- GS-ChopShop (Advanced) Client
-- ox_target part-based chop + search radius
-- V2 polish: VIN + plate steps support
--========================================
...
-- Forward declarations (Lua does NOT hoist locals into earlier closures)
local getNextRequiredStep
local getAnimForStep

local function mechDo(veh, stepKey, label, baseMs, boneName)
  -- Mechanics module may load after this file depending on manifest ordering.
  -- Always fetch from the global so the feature doesn't silently break.
  local Mechanics = _G.GS_CHOP_MECH
  if not Mechanics or not Mechanics.DoStep then
    return false, 'missing'
  end

  local d, a = getAnimForStep(stepKey)
  local ok, reason = Mechanics.DoStep({
    veh = veh,
    stepKey = stepKey,
    label = label,
    baseMs = baseMs,
    sessionTimeMult = (activeSession and activeSession.timeMult) or 1.0,
    animDict = d,
    animName = a,
    bone = boneName,
  })
...
-- Define early so any ox_target callbacks can safely reference it.
...
-- Some older code paths reference this as a global.
...
local function doSkill(stepKey)
  ...
  -- Per-part patterns (requested)
  -- Wheels: quick 2-step
  -- Doors: timing bar feel (single medium)
  -- Engine: harder multi-stage
  -- Plates: short easy
  local profile
  local keys = { 'w', 'a', 's', 'd' }
...
-- Compatibility alias: older step handlers call skillCheck(stepKey)
...
getAnimForStep = function(stepKey)
  -- Unified animation mechanic (requested): one consistent "garage op" loop for every step.
  -- Keeps timing + feel consistent across the whole flow.
...
local function allNonFinalDoneClient()
  ...
  -- Prefer server-provided required steps (vehicle-aware)
...
local function stepEnabled(stepKey)
  -- Smarter-contract modifiers can force certain steps even if toggles are off.
  local function contractForces(k)
    if not activeContract or not activeContract.modifiers then return false end
    k = tostring(k)
    -- Map modifier ids to forced steps (client-side mirror).
    local map = {
...
-- Compute which steps actually apply to this specific vehicle (e.g. 2-door cars have no rear doors).
-- Server cannot call GetEntityBoneIndexByName, so we send this list when starting the chop.
    ...
    if stepEnabled(step.key) then
      opts[#opts+1] = {
        ...
        onSelect = function()
          ...
          local ok, reason = mechDo(veh, step.key, step.label, step.time, (step.bones and step.bones[1]) or nil)
          if not ok then
  ...
  for _, part in ipairs(PARTS) do
    if isRequired(part.key) and boneExists(part.bone) then
      ...
      onSelect = function()
        ...
        local ok, reason = mechDo(veh, part.key, part.label, part.time, part.bone)
        if not ok then
          if reason == 'skill' then
            TriggerServerEvent('gs-chopshop:server:noteSkillFail')
            notify('Failed. Try again.', 'error')
        ...
        -- client visuals immediately, server validates + acks
        -- visuals applied on server ack (prevents out-of-order visual desync)
  ...
  if Config.V3 and Config.V3.enableMoveCutCrush and isRequired('cut_shell') and boneExists('bonnet') then
    opts[#opts+1] = {
      ...
      onSelect = function()
        ...
        -- Arcade-mechanical cut: quick skill + quick progress, still feels like welding.
        local ok, reason = mechDo(veh, 'cut_shell', 'Cutting shell', 2800, 'bonnet')
        if not ok then
          if reason == 'skill' then
            TriggerServerEvent('gs-chopshop:server:noteSkillFail')
            notify('Failed. Try again.', 'error')
  ...
  if not useConverter then
    opts[#opts+1] = {
      ...
      onSelect = function()
        ...
        local veh = getVehFromNet(sessionNetId)
        local ok, reason = mechDo(veh, 'final', 'Disposing shell', 2500, nil)
        if not ok then
          if reason == 'skill' then
            TriggerServerEvent('gs-chopshop:server:noteSkillFail')
            notify('Failed. Try again.', 'error')
...
local function rebuildBayZones()
  ...
  for i, bay in ipairs(bays) do
    local id = exports.ox_target:addSphereZone({
      ...
      debug = false,
      options = {
        -- V3 workflow: align shell on cut line (on-foot so it's always visible)
        {
          ...
          onSelect = function()
            ...
            if dist > rad then
              ...
              return
	            ...
	            inPartAction = true
	            local ok, reason = mechDo(veh, 'move_shell', 'Aligning shell', 2000, nil)
	            if not ok then
	              if reason == 'skill' then
	                TriggerServerEvent('gs-chopshop:server:noteSkillFail')
	                notify('Failed. Try again.', 'error')
...
-- (getNextRequiredStep defined near the top of this file)
...
RegisterNetEvent("gs-chopshop:client:receiveData", function(data)
  ...
  if data and data.session then
    -- Do NOT blindly overwrite the whole table or you can lose the locally-computed requiredList.
    activeSession = activeSession or {}
  ...
  -- Tool Mastery snapshot (individual)
  if data and data.toolMastery and _G.GS_CHOP_MECH and _G.GS_CHOP_MECH.SetMastery then
    _G.GS_CHOP_MECH.SetMastery(data.toolMastery)
  end

  -- Bay visual evolution (client-only props)
  if _G.GS_CHOP_VIS and _G.GS_CHOP_VIS.ApplyFromData then
    _G.GS_CHOP_VIS.ApplyFromData(data, activeSession)
  end
end)

-- Live mastery updates
RegisterNetEvent('gs-chopshop:client:toolMastery:set', function(m)
  if _G.GS_CHOP_MECH and _G.GS_CHOP_MECH.SetMastery then
    _G.GS_CHOP_MECH.SetMastery(m)
  end
end)
...
--========================================
-- FOUND VEHICLE DETECTOR (ONE THREAD ONLY)
--========================================
CreateThread(function()
  while true do
    Wait(600)
...
--========================================
-- V4 GUIDE MARKERS (NEXT STEP ARROWS)
--========================================
-- getNextRequiredStep() is defined earlier (before events) to avoid Lua forward-reference issues.
...
-- Auto-begin chop when driving the contract vehicle into a bay (no target needed)
CreateThread(function()
  while true do
    Wait(350)
...
RegisterNetEvent('gs-chopshop:client:coopInviteNearest', function()
  notify('Co-op removed. Invite players to your Syndicate instead.', 'inform')
end)
